...
export const useLoanActions = () => {
	...
	const allLoansStatus = useAppSelector(
		(state) => state.loans.status
	);
	const loan = useAppSelector((state) => state.loans.loan);
	const loanStatusUpdate = useAppSelector(
		(state) => state.loans.statusUpdate
	);
	const loanStatus = useAppSelector(
		(state) => state.loans.statusLoan
	);
	const dateAvailable = useAppSelector(
		(state) => state.loans.dateAvailable
	);
